<?php

return [

    /*
    |--------------------------------------------------------------------------
    | Authentication Language Lines
    |--------------------------------------------------------------------------
    |
    | The following language lines are used during authentication for various
    | messages that we need to display to the user. You are free to modify
    | these language lines according to your application's requirements.
    |
    */
    'google_login_required' => 'هذا الحساب مسجل باستخدام Google، يرجى تسجيل الدخول عبر Google.',

    'cannot_set_password' => 'لا يمكن إنشاء كلمة مرور لهذا الحساب.',

    'google_account' => 'هذا الحساب مسجل باستخدام Google، يرجى إنشاء كلمة مرور أولاً.',

    'password_set_success' => 'تم إنشاء كلمة المرور بنجاح.',

    'password_already_set' => 'كلمة المرور موجودة بالفعل، استخدم تغيير كلمة المرور.',

    'too_many_requests' => 'لقد تجاوزت عدد المحاولات، حاول لاحقاً.',

    'unauthorized' => 'ليس لديك صلاحية لتنفيذ هذا الإجراء.',

    'account_inactive' => 'حسابك غير نشط.',

    'account_suspended' => 'تم تعليق حسابك.',

    'login_successfully' => 'تم تسجيل الدخول بنجاح',

    'invalid_google_token' => 'رمز تسجيل الدخول عبر Google غير صالح.',

    'google_email_not_found' => 'تعذر الحصول على البريد الإلكتروني من حساب Google.',

    'google_email_not_verified' => 'البريد الإلكتروني في حساب Google غير مُحقق.',

    'user_not_found' => 'المستخدم غير موجود.',

    'email_exists' => 'البريد الإلكتروني مستخدم بالفعل.',

    'email_verified' => 'تم تفعيل البريد الإلكتروني بنجاح.',

    'email_already_verified' => 'تم تفعيل البريد الإلكتروني بالفعل.',

    'invalid_otp'=>'رمز التحقق غير صحيح.',

    'expired_otp' => 'انتهت صلاحية رمز التحقق. يرجى طلب رمز جديد.',

    'password_reset_success' => 'تم إعادة تعيين كلمة المرور بنجاح.',

    'otp_sent' => 'تم إرسال رمز التحقق بنجاح.',

    'email_not_found' => 'البريد الإلكتروني غير موجود.',

    'register_success' => 'تم إنشاء الحساب، يرجى تأكيد البريد الإلكتروني.',

    'login_success' => 'تم تسجيل الدخول بنجاح',

    'logout_success' => 'تم تسجيل الخروج بنجاح',

    'invalid_credentials'=>'بيانات الدخول غير صحيحة.',

    'account_disabled' => 'الحساب غير نشط، يرجى التواصل مع الإدارة.',

    'email_required'=>'البريد الإلكتروني مطلوب.',

    'email_invalid'=>'البريد الإلكتروني غير صحيح.',

    'password_required'=>'كلمة المرور مطلوبة.',

    'profile_success'=>'تم جلب بيانات المستخدم بنجاح.',

    'unauthenticated' => 'يجب تسجيل الدخول أولاً.',

    'profile_updated' => 'تم تحديث الملف الشخصي بنجاح.',

    'invalid_current_password' => 'كلمة المرور الحالية غير صحيحة.',

    'password_changed' => 'تم تغيير كلمة المرور بنجاح.',

    'failed' => 'These credentials do not match our records.',

    'password' => 'The provided password is incorrect.',

    'throttle' => 'Too many login attempts. Please try again in :seconds seconds.',

];
